...
Our guiding claim is simple. Once the cognitive measures are treated as fallible indicators of a shared cognitive dimension, the musician versus nonmusician contrast is most parsimoniously described as a difference on a general latent factor, `g`, plus a limited amount of remaining task-specific variance, most clearly for Melody span, and more modestly for vocabulary in the multilevel specification.

To keep this supplement readable, we report only the information that is substantively necessary for evaluating that claim. Full raw console output is intentionally omitted from the main document. The tables in the `/Tables` subfolder and the figures in the `/Figures` subfolder are used directly below.

# Data and preliminary checks

The reanalysis used the same matched sample reported in the target article, namely 600 musicians and 600 nonmusicians. Before moving to latent models, it is useful to document the matching variables and the main covariate included in the SEMs.
...
The commentary argues that interpretation should start from the well-known tendency of cognitive measures to correlate positively. In the present dataset this premise is straightforwardly supported. Raven, vocabulary, n-back, digit span, spatial span, and melody span all show mostly positive associations, and SES is also positively related to the cognitive indicators. In other words, the data exhibit exactly the kind of positive manifold that motivates a common-factor representation.
...
Second, because the original study was multilab and cross-national, we fitted a **multilevel SEM** with clustering by country. This model cannot reproduce the exact multigroup design simultaneously, but it provides a useful converging test. The crucial question here was whether the group effect is best placed only on the observed variables, only on `g`, or on `g` plus a very small number of targeted residual paths.
...
```{r fig-s1, fig.cap="Figure S1. Preferred multigroup representation. Dual values separated by a slash denote the two groups for group-specific quantities such as residual variances or residual covariances."}
knitr::include_graphics("../Figures/Figure1_MG.png")
```

Figure S1 summarizes the final substantive picture. The group contrast is represented in two places. First, musicians are higher on the latent factor `g`. Second, Melody span shows a remaining indicator-specific intercept difference that is not absorbed by `g`.
...
The latent mean difference was approximately 0.51 latent SD units, which is not a task-specific effect but a general-factor effect. Because the latent variance of `g` was fixed to 1 in both groups, this contrast can be read directly as a difference of about half a latent standard deviation. At the same time, Melody span retained an additional intercept shift of roughly 0.85 units in the standardized observed metric. Thus, the musical-memory task is not simply another interchangeable indicator of the same group contrast, it contains an obvious residual advantage beyond the general cognitive factor.

The SES effect on `g` was small and positive in both groups, and the standardized coefficients were very similar. This supports the decision to retain a common SES regression in the preferred multigroup model.

## Interpretation of the multigroup findings

The multigroup sequence supports a layered interpretation.

The broad musician advantage in the cognitive battery is largely captured by a single latent dimension. That is the main result. However, the failure of full scalar invariance also shows that not every group difference should be read as a pure reflection of `g`. Melody span carries additional group-related variance, which is exactly what one would expect from a task that is closest to musical expertise.

This pattern is theoretically preferable to a flat task-by-task reading. Instead of saying that musicians have separate advantages in several partially disconnected memory tasks, the latent model suggests a more economical account: a general cognitive difference plus one especially salient music-specific residual deviation.
...
This result is useful because it converges with the multigroup CFA while not being identical to it. The multilevel model still says that the main group contrast sits at the common-factor level. Yet it also suggests that a small amount of indicator-specific variance remains, concentrated especially in Melody span and, to a lesser extent, vocabulary.
...
This matters for interpretation. The original empirical facts remain intact, but their organization changes. The results do not force a fragmented task-by-task narrative. They are better summarized as evidence for a broad latent difference, with task-specific excess concentrated where theory would most expect it, namely in the music-relevant indicator.

# Reproducibility note

This `.Rmd` file is concise and does not feature all R analyses that were performed. The full  . It uses the precomputed tables stored in `/Tables` and the diagrams stored in `/Figures`, rather than re-running the full SEM workflow during knitting. This keeps the supplementary document easy to audit and stable across machines.

The underlying analyses were based on the same dataset used in the commentary project and were originally carried out in `lavaan`. Full scripts, raw console outputs, and intermediate model objects can be archived separately if desired. The present supplement is designed to be the readable technical companion to the commentary, not the full analysis log.
